  ...
  for (var i = 0; i < cuerpo.hijos.length; i++) {
    ...
    switch(sent.nombre)
    {
      ...
      case Const.SI:
        ...
        ambEjecucion.push(sent.nombre + contAmbActual);
        contAmbActual++;
        ...
        {//se procede a colocar las etiquetas de verdad y falso en su lugar
          ...
          if (sent.hijos.length === 3) {//posee un ELSE
            ambEjecucion.pop();
            ambEjecucion.push("ELSE" + contAmbActual);
            contAmbActual++;
            l = getEtq();
...
function buscarVariable(id) {
  ...
  var amb;
  var contAmb = ambEjecucion.length;
  var tamTotal = 0;
  var t = "p";
